...
const siteinfoService = {
  ...
  findOne: async (req, res) => {
    try {
      let result = await RegionInfo.findById(req.params.id).select("-__v");
  ...
  find: async (req, res) => {
    try {
      let result = await RegionInfo.find({}).select("-__v");
  ...
  post: async (req, res) => {
    ...
    try {
      var newRegionInfo = new RegionInfo(contentBody);

      let result = await newRegionInfo.save();
   ...
   */
  delete: (req, res) => {
    RegionInfo.deleteOne({ _id: req.params.id })
  ...
  update: (req, res) => {
    ...
    RegionInfo.updateOne({ _id: req.params.id }, { $set: final })
      .then((result) => res.status(200).json({ success: true, result }))
      .catch((error) =>
        res
          .status(500)
